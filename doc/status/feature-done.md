...
| 2026-04-18 17:56 | E9-S2-I1 | 完成截图解析链路 SystemPrompt 注入优先级落地（业务类型 > 全局 > 默认）及 OCR确认后透传至 AI 阶段 | `pytest tests/test_e9_s2_i1_analysis_system_prompt.py` 4通过；`pytest tests/test_e9_s1_i1_system_prompt_config.py tests/test_e9_s2_i1_analysis_system_prompt.py tests/test_e9_s2_i2_chat_system_prompt.py` 12通过 | 当前仅记录命中来源与长度，未输出完整提示词内容，后续可扩展更细粒度链路观测 |
| 2026-04-18 17:57 | E9-S2-I2 | 完成对话链路全局 SystemPrompt 注入与默认回退，兼容剔除上游旧 system 消息，支持设置变更后立即生效 | `pytest tests/test_e9_s2_i2_chat_system_prompt.py` 4通过；`pytest tests/test_e8_s2_i2_chat_service.py tests/test_e8_s2_i2_chat_pipeline.py tests/test_e8_s2_i2_chat_window_send_state.py` 10通过；`pytest` 全量90通过 | 运行中更新依赖每次请求实时读配置，若后续引入缓存需同步更新失效策略 |
